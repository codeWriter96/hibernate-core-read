...
public interface Session extends SharedSessionContract, EntityManager, HibernateEntityManager, AutoCloseable, Closeable {
	 ...
	 */
	//保存当前对象，返回序列化后的主键。立即触发insert语句，从数据库获得主键的值（OID值）
	 ...
	 */
	// merge当前对象。
	// 如果没有对象，则保存当前对象；如果有对象判断数据库内的对象是否与传入的对象一致，一致则结束，不一致则更新整个对象（调了一次查询 select * ！！！）
	 ...
	 */
	//清除当前session中的数据。并取消所有挂起的保存、更新和删除
	void clear();
	 ...
	 */
	//添加SessionEventListener.功能类似spring的TransactionEventListener作用
	//Hibernate中的事件监听机制可以对Session对象的动作进行监听，一旦发生了特殊的事件，Hibernate就会执行监听器中的事件处理方法
	// https://www.cnblogs.com/suncj/p/4338048.html
